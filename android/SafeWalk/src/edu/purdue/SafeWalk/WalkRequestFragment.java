...
//		return mTouchView;
	...
	private void initMap() {
		...
		mMap = ((CustomMapFragment)getFragmentManager().findFragmentById(R.id.map)).getMap();
	...
	public void onPopUpBubbleClick(View v){
		...
		{
			latlng = ((CustomMapFragment) getFragmentManager().findFragmentById(R.id.map)).dropPinAtCenter(getActivity(), "Start", BitmapDescriptorFactory.HUE_GREEN);
			
			SharedPreferences bubbleState = getActivity().getSharedPreferences("bubbleState", Context.MODE_PRIVATE);
		...
		{
			latlng = ((CustomMapFragment) getFragmentManager().findFragmentById(R.id.map)).dropPinAtCenter(getActivity(), "End", BitmapDescriptorFactory.HUE_RED);
